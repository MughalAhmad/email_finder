...
// // ✅ FIX: Define validateEmail function at the top level
...
//   // Skip if it looks like a file (contains .webp, .png, .jpg, etc.)
...
//   // Skip if it looks like a system ID (contains random hex strings)
...
//   const isValid = validateEmail(cleanEmail); // ✅ Use the validateEmail function
...
//     // ✅ Now validateEmail is defined and accessible
...
//       let user = { appPassword: 'qfqs jbwv ieaa rkbh' }; // For testing only - move to database
...
//         if (index === 0) {
//           subject = template.title || 'Email from ' + fromEmail;
//         }
//         combinedHtml += `<div style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: ${index < templates.length - 1 ? '1px solid #e0e0e0' : 'none'};">`;
//         combinedHtml += `<h2 style="color: #2563eb; margin-bottom: 10px;">${template.title}</h2>`;
//         combinedHtml += `<div>${template.content || template.body || ''}</div>`;
//         combinedHtml += `</div>`;
//       });
